        ...
        public void OnComplete(Android.Gms.Tasks.Task task)
        {
            if (task.IsSuccessful)
            {
                ...
                if(result is DocumentSnapshot doc)
                {
                    ...
                    user.Id = int.Parse(doc.Id);
                    user.nickname = doc.GetString("Nickname");
                    user.email = doc.GetString("Email");
                    user.budget = (double)doc.GetDouble("Budget");
                    user.inviteCode = doc.GetString("InviteCode");
                    user.saldoId = int.Parse(doc.GetString("SaldoId"));
                    user.selectedPlan = int.Parse(doc.GetString("SelectedPlan"));
                    _tcs.SetResult(user);
                    return;
